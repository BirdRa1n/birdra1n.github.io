...
import {
  FiHome, FiFolder, FiFileText,
  FiMail, FiAward, FiLogOut, FiMenu, FiX,
  FiUser, FiChevronRight, FiSmartphone, FiTag,
} from "react-icons/fi";
        ...
        >
          <NextLink href="/admin" className="flex items-center gap-2">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span
                className="text-[9px] tracking-widest opacity-40"
          ...
          <button
            className="lg:hidden opacity-50 hover:opacity-100"
            onClick={onClose}
          >
            <FiX size={18} />
          </button>
...
export default function AdminLayout({
  ...
  }, [isAdmin, isLoading, isLoginPage]); // eslint-disable-line react-hooks/exhaustive-deps

  // Página de login: renderiza direto sem guards
  if (isLoginPage) return <>{children}</>;

  // Carregando auth: mostra spinner simples
  if (isLoading) {
    return (
      <div
  ...
  // Não autenticado: null enquanto o router.replace processa
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <a
              href="/"
              target="_blank"
              className="text-[10px] tracking-widest opacity-30 hover:opacity-60 transition-opacity"
              style={{ fontFamily: "var(--font-mono)", color: "var(--neon)" }}
            >
              VIEW_SITE ↗
            </a>
